...
################################################################################
#This script is used to produce Supplemental Figures 9 and 10
#
#Run under R 3.6.1
#This script takes as arguments:
# outDir: Path where files are to be written
# theMatrix_nonCGI: Table containing information about which TFs had a peak 1kb upstream
#     of the TSS of a given gene (restricted to non-CGI genes), and the TPM of that gene.
#     Provided for download as Binding_Expression_Matrix_nonCGI.txt
# theMatrix_anyCGI: Table containing information about which TFs had a peak 1kb upstream
#     of the TSS of a given gene (restricted to CGI genes), and the TPM of that gene.
#     Provided for download as Binding_Expression_Matrix_anyCGI.txt
#
################################################################################
...
outDir <- args[1]
theMatrix_nonCGI <- args[2]
theMatrix_anyCGI <- args[3]
...
theMatrix <- read.delim(theMatrix_nonCGI, header=T, sep="\t", stringsAsFactors=F)
theMatrix <- theMatrix[!is.na(theMatrix[,"TPM"]),]
theMatrix <- theMatrix[,5:ncol(theMatrix)]
theMatrix[,1] <- log(theMatrix[,1]+0.1)

saveBase <- paste(outDir, "Supplemental_10", sep="")
...
theMatrix <- "/cluster/home/bmoyers/ENCODE_500Plus/Binding_Expression_Matrix_anyCGI_2022July13.txt"
theMatrix <- read.delim(theMatrix, header=T, sep="\t", stringsAsFactors=F)
theMatrix <- theMatrix[!is.na(theMatrix[,"TPM"]),]
theMatrix <- theMatrix[,5:ncol(theMatrix)]
theMatrix[,1] <- log(theMatrix[,1]+0.1)

saveBase <- paste(outDir, "Supplemental_9", sep="")
fitLM(theMatrix, saveBase)
