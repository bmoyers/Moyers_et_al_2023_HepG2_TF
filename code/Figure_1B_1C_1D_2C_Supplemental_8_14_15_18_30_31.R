#!/usr/bin/env R
#Figure_1B_1C_1D_2C_Supplemental_8_14_15_18_30_31.R

################################################################################
#This script is used to produce Figures 1B, 1C, 1D, and 2C
#as well as Supplemental Figures 8, 14, 15, 18, 30, and 31.
#
#Run under R 4.1.0
...
for (i in 1:3) {
  ...
  saveFile <- paste(outDir, "Supplemental_31_rep", i, ".pdf", sep="")
...
for (i in 1:(length(element_rep_list)-1)) {
  ...
  for (j in (i+1):(length(element_rep_list))) {
    ...
    saveFile <- paste(outDir, "Supplemental_30_rep", i, "_v_rep", j, "_bothRuns.pdf", sep="")
...
saveFile <- paste(outDir, "Supplemental_X_Numbound_Activity.pdf", sep="")
...
saveFile <- paste(outDir, "Supplemental_X_Numbound_GC.pdf", sep="")
...
saveFile <- paste(outDir, "Supplemental_8.pdf", sep="")
...
saveFile <- paste(outDir, "Supplemental_15_table.txt", sep="")
...
numbersTable <- table(this_repressors[,c("DAPsBound", "Class")])
saveFile <- paste(outDir, "Supplemental_14_repressorsTable.txt", sep="")
write.table(numbersTable, saveFile, row.names=T, col.names=T, sep="\t", quote=F)
numbersTable <- read.table(saveFile, header=T, sep="\t", stringsAsFactors=F)
...
numbersTable <- table(this_random[,c("DAPsBound", "Class")])
saveFile <- paste(outDir, "Supplemental_14_randomTable.txt", sep="")
write.table(numbersTable, saveFile, row.names=T, col.names=T, sep="\t", quote=F)
numbersTable <- read.table(saveFile, header=T, sep="\t", stringsAsFactors=F)
...
numbersTable <- table(this_activators[,c("DAPsBound", "Class")])
saveFile <- paste(outDir, "Supplemental_14_activatorsTable.txt", sep="")
write.table(numbersTable, saveFile, row.names=T, col.names=T, sep="\t", quote=F)
numbersTable <- read.table(saveFile, header=T, sep="\t", stringsAsFactors=F)
...
saveFile <- paste(outDir, "Supplemental_18.pdf", sep="")
...
saveFile <- paste(outDir, "Supplemental_18_table.txt", sep="")
...
saveFile <- paste(outDir, "Supplemental_18_tableObservations.txt", sep="")
