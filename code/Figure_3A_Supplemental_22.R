#!/usr/bin/env R
#Figure_3A_Supplemental_22.R


################################################################################
#This script is used to produce Figure 3A as well as Supplemental Figure 22.
#
#Run under R 3.6.1
...
theCutoff <- floor(length(centered_peaks)/4)
...
p <- ggplot(connections_df[connections_df$connections<=6,], aes(x=type, y=connections)) + blank_theme +
...
for (i in 1:length(peaks)) {
  theName <- strsplit(peaks[i], split="/")[[1]][length(strsplit(centered_peaks[i], split="/")[[1]])]
  theName <- strsplit(theName, split="_")[[1]][1]
  theName <- gsub("-FLAG", "", theName)
  theName <- gsub("-eGFP", "", theName)
...
b <- cbind(data_frame_abc[,c("HOT", "randomized_Jaccard")], rep("randomized", nrow(data_frame_abc)))
...
saveFile <- paste(outDir, "Supplemental_22.pdf", sep="")
split_plot<-ggplot(graphDF, aes(x=HOT, y=Jaccard, fill=Case)) + blank_theme + scale_fill_manual(name="Category", values = custom_col, na.value="grey50") +
