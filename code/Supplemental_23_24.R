#!/usr/bin/env R
#Supplemental_23_24.R

################################################################################
#This script is used to produce Supplemental Figures 23 and 24
...
for (i in 1:length(peaks)) {
  ...
  if(file.exists(these_passed_motifs)) {
...
for (k in 1:length(cgiTypes)) {
  ...
  this_motif_connections_infoTable_all <- this_motif_connections_infoTable_all[this_motif_connections_infoTable_all[,1]%in%LambertTFs[LambertTFs[,5]=="TF",2],]
  if(nrow(this_motif_connections_infoTable_all)>=3) {
    ...
    saveFile <- paste(outDir, "Distal_All_logRatio_promVenh_cgiAware_", cgiTypes[i], ".txt", sep="")
