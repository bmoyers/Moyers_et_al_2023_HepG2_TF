...
################################################################################
#This script is used to produce Figures 4C, 4D, and 4E.
#
#Run under R/4.1.1
...
#
#Note that this script also expects the following in the user's PATH:
# python 2.7.15
...
running_meme_pipeline <- function(thePeaks, hotSites_gr, genome, this_outDir) {
  ...
  theCommand <- paste("python nullseq_generate.py -x 1 -m 1000 -r 1 -o ", control_file, " ", saveFile, " hg38 hg38_indices/", sep="")
  system(theCommand)
  ...
  theMotifs <- runMeme(input=inputSeqs, control = controlSeqs, outdir = meme_outDir, alph = "dna",
    silent = FALSE, nmotifs=5, minw=6, maxw=50, objfun="de")
