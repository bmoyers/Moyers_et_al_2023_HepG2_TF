#!/usr/bin/env R
#Figure_2B_Supplemental_26.R

################################################################################
#This script is used to produce Figure 2B as well as Supplemental Figure 26.
#
#Run under R 4.0.0
...
#cd /cluster/home/bmoyers/Figures/ENCODE_500plus/Results_post_July5/PaperFigures_2023February21/
...
saveFile_1 <- paste(outDir, "Supplemental_26.pdf", sep="")
#png(filename = saveFile_1, width = 1800, height = 1000, units = "px", res = 150, type="cairo")
pdf(saveFile_1)
ggplot(data=finalDF[finalDF[,5]<=100,], aes(x=label, y=pearson_cor)) + theme_bw() +
...
p <- ggplot(graphDF_combined, aes(x=Order, y=estimate, fill=frac_sig))  + theme_bw() + xlab("TF") + ylab("Model Estimate") +
