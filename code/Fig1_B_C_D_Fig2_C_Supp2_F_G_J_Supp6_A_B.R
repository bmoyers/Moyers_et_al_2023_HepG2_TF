#Fig1_B_C_D_Fig2_C_Supp2_F_G_J_Supp6_A_B.R
...
# the_info_table: Supplementary Table 6
...
for (i in 1:3) {
  ...
  saveFile <- paste(outDir, "Supplemental_Fig_S6B_rep", i, ".pdf", sep="")
  p <- ggplot(thisSet, aes(x=n_obs_bc)) + geom_histogram() + ylab("Frequency") + xlab("Barcodes per element") +
...
for (i in 1:(length(element_rep_list)-1)) {
  for (j in (i+1):(length(element_rep_list))) {
    element_mini_repMatrix <- cbind(element_mini_repMatrix[[i]][,"log2"], element_mini_repMatrix[[j]][,"log2"])
    ...
    saveFile <- paste(outDir, "Supplemental_Fig_S6A_re", i, "_v_rep", j, "_bothRuns.pdf", sep="")
    p <-  ggplot(element_mini_repMatrix, aes(x=first, y=second)) + geom_point(alpha=0.3) + geom_point(data=negative_mini_repMatrix, colour = "red") + geom_point(data=positive_mini_repMatrix, colour = "blue") + ggtitle(theTitle) +
      ...
      geom_abline(slope=1, intercept=0, color='purple') + xlab(colnames(mini_repMatrix)[i]) + ylab(colnames(mini_repMatrix)[j])
    ggsave(saveFile)
...
graphDF <- cbind(combinedMat$name, combinedMat$log2, numBound, theBins)
graphDF <- as.data.frame(graphDF)
colnames(graphDF) <- c("name", "Signal", "numBound", "DAPsBound")
...
alt_controlDF <- data.frame(name=controls_posNeg$name, Signal=controls_posNeg$log2, numBound=rep(NA, nrow(controls_posNeg)), DAPsBound=controls_posNeg$label, category=rep("Control", nrow(controls_posNeg)))
...
p <- ggplot(this_graphDF_notiling, aes(x=DAPsBound, y=Signal, fill=category)) + theme_bw() + geom_boxplot() + ylab("MPRA Signal") + xlab("DAPs Bound") +
...
graphDF_promoters <- rbind(graphDF_promoters, alt_controlDF[,1:4])
...
graphDF_distal <- graphDF[-unique(distalIntersect[,1]),]
graphDF_distal <- rbind(graphDF_distal, alt_controlDF[,1:4])
...
p <- ggplot(abc_genes_graphDF, aes(x=DAPsBound, y=logTPM)) + theme_bw() + geom_boxplot() + xlab("DAPs Bound") + ylab("Connected Gene log(TPM)") + ylim(-2.5,7.5) +
...
p <- ggplot(quickCheckDF, aes(x=DAPsBound, y=all_fraction_retained)) + theme_bw() + geom_bar(stat="identity") + ylab("Fraction with ABC Loop") + xlab("DAPs Bound") +
  theme(axis.text= element_text(size=20), axis.title=element_text(size=25), axis.text.x=element_text(angle=90, vjust=0.5, hjust=0.5))
ggsave(saveFile)
...
this_graphDF <- rbind(graphDF, alt_controlDF[,1:4])
...
saveFile <- paste(outDir, "Supplemental_Fig_S2G.pdf", sep="")
p_repressors_noRest <- ggplot(this_graphDF_promoters, aes(x=DAPsBound, y=Signal, fill=RepressorBound_noRest)) + theme_bw() + geom_boxplot() +
...
colnames(this_graphDF)[5] <- "OtherBound"
...
p <- ggplot(combined_graphDF, aes(x=DAPsBound, y=Signal, fill=Class)) + theme_bw() + geom_boxplot() + ylab("MPRA Signal") + xlab("DAPs Bound") +
  theme(axis.text= element_text(size=20), axis.title=element_text(size=25), axis.text.x=element_text(angle=90, vjust=0.5, hjust=0.5), legend.key.size=unit(1, 'cm'), legend.title=element_text(size=25), legend.text=element_text(size=20))  +
...
saveFile <- paste(outDir, "Supplemental_Fig_S2F.pdf", sep="")
p <- ggplot(fun_new_graphDF, aes(x=DAPsBound, y=Signal, fill=Class)) + geom_boxplot() + theme_bw() + ylab("MPRA Signal") + xlab("DAPs Bound") +
  theme(axis.text= element_text(size=20), axis.title=element_text(size=25), axis.text.x=element_text(angle=90, vjust=0.5, hjust=0.5), legend.key.size=unit(1, 'cm'), legend.title=element_text(size=25), legend.text=element_text(size=20))  +
  ylim(-2,3.5) + scale_fill_manual(name="Category", values = custom_col, na.value="grey50") +
...
for (l in 1:length(all_of_interest)) {
  ...
  for (i in uniqueNumbers) {
    ...
    if(i==1) {
      ...
      thisDF <- data.frame(TF=rep(all_of_interest[l], nrow(this_observedSet)), label=rep(label, nrow(this_observedSet)), obsSignal=this_observedSet$log2, compSignal=this_controlSet$log2)
...
saveFile <- paste(outDir, "Supplemental_Fig_S2J.pdf", sep="")
p <- ggplot(all_paired_observations, aes(x=TF, y=difference, fill=label)) + theme_bw() + geom_boxplot(outlier.shape=NA) + ylim(-2,2) +
  theme(axis.text= element_text(size=20), axis.title=element_text(size=25), axis.text.x=element_text(angle=90, vjust=0.5, hjust=0.5), legend.key.size=unit(1, 'cm'), legend.title=element_text(size=25), legend.text=element_text(size=20)) +
  scale_fill_manual(name="Category", values = custom_col, na.value="grey50") + ylab("Paired Signal Difference") +
  geom_hline(yintercept=0) + geom_text(data=data.frame(), aes(x=the_ttest_table$TF, y=rep(1.5, nrow(the_ttest_table)), label=the_ttest_table$label), col='purple', size=10, angle = 90) #+ coord_flip()
ggsave(saveFile, width=18)

saveFile <- paste(outDir, "ManyTFs_explorationBoxplot_promotersOnly_Obs_v_Comparable_difference_table_bothRuns.txt", sep="")
write.table(all_paired_observations, saveFile, row.names=F, col.names=T, sep="\t", quote=F)

saveFile <- paste(outDir, "ManyTFs_explorationBoxplot_promotersOnly_Obs_v_Comparable_difference_ttest_table_bothRuns.txt", sep="")
