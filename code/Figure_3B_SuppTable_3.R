#!/usr/bin/env R
#Figure_3B_SuppTable_3.R


################################################################################
#This script is used to produce Figure 3B as well as Supplemental Table 3.
#
#Run under R 4.1.0
...
for (i in 1:length(peaks)) {
  ...
  if(file.exists(these_passed_motifs)) {
    ...
    if(length(theMotifs)>0) {
      ...
      confirmation_Hits <- runFimo(sequences=theSequences, motifs=theMotifs)
...
saveFile <- paste(outDir, "SuppTable3.txt", sep="")
...
saveFile <- paste(outDir, "Distal_All_logRatio_promVenh_min100Cases.txt", sep="")
