#!/usr/bin/env R
#Supplemental_26_27.R

################################################################################
#This script is used to produce Supplemental Figures 26 and 27
...
for (k in 1:length(cgiTypes)) {
  ...
  if(nrow(this_motif_connections_infoTable_all)>=3) {
    ...
    saveFile <- paste(outDir, "Distal_All_logRatio_promVenh_cgiAware_", cgiTypes[k], ".txt", sep="")
