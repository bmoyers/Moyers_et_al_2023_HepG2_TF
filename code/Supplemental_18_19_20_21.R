#!/usr/bin/env R
#Supplemental_18_19_20_21.R

################################################################################
#This script is used to produce Supplemental Figures 18, 19, 20, and 21
#
#Run under R 3.6.1
...
createBoundMatrix <- function(allFiles, chromSizes, theSplit="_", windowSize=2000, exclusionList) {
  ...
  blackIntersect <- as.data.frame(findOverlaps(theRegions_gr, exclusionList_gr))
  if(nrow(blackIntersect)>0) {
    chromDF <- chromDF[-unique(blackIntersect[,1]),]
    chromDF <- as.data.frame(chromDF)
...
################################################################################
graphPC_info <- function(thisDF, saveFile, xlabel, ylabel, measure) {
  ...
  if(!is.factor(thisDF[,"Measure"])) {
    mini_thisDF <- thisDF[!is.na(thisDF[,"Measure"]),]
    breaks1 <- seq(floor(min(mini_thisDF[,1])),ceiling(max(mini_thisDF[,1])), by=0.5)
    breaks2 <- seq(floor(min(mini_thisDF[,2])),ceiling(max(mini_thisDF[,2])), by=0.5)
...
saveFile <- paste(outDir, "Supplemental_21.pdf", sep="")
...
saveFile <- paste(outDir, "Supplemental_18.png", sep="")
thisDF <- graphDF[,c("prc1", "prc2", "ABC")]
colnames(thisDF) <- c("firstPRC", "secondPRC", "Measure")
graphPC_info(thisDF, saveFile, lab_vec[1], lab_vec[2], "ABC")


saveFile <- paste(outDir, "Supplemental_19.png", sep="")
thisDF <- graphDF[,c("prc1", "prc2", "numBound")]
colnames(thisDF) <- c("firstPRC", "secondPRC", "Measure")
graphPC_info(thisDF, saveFile, lab_vec[1], lab_vec[2], "numBound")


saveFile <- paste(outDir, "Supplemental_20.png", sep="")
